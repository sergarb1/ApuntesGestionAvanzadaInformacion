...
Después de instalar el plugin de cookies, sigue el asistente de configuración seleccionando España como país. El banner se personaliza en "Diseño". La página de aviso legal debe incluir datos fiscales, registro mercantil y condiciones de uso.
...
El comando openssl req -new -x509 genera el certificado y la clave. Luego edita el archivo httpd-ssl.conf para apuntar a esos archivos. Recuerda que los navegadores mostrarán advertencia de seguridad al ser autofirmado.
...
El registro CNAME en la zona DNS apunta el subdominio al dominio principal. En cPanel, crea el subdominio y especifica la carpeta de destino. Usa nslookup para verificar la resolución DNS.
...
En UpdraftPlus, programa los respaldos en la pestaña "Ajustes de respaldo". Para Google Drive, necesitarás autenticar la cuenta. La retención se configura en el mismo panel. No olvides probar la restauración — la copia solo sirve si puedes recuperar los datos.
